...
import { Plus, X, Pencil, Trash2, Copy, Eye } from 'lucide-react'; 
...
export default function CategoryList() {
    ...
    // --- API & Core Functions (Optimized for brevity, but functionality is preserved) ---
    ...
    const handleFileUpload = async (e, fieldName) => {
        ...
        try {
            ...
            if (result?.message === "Files uploaded" && result.files) {
                ...
                e.target.value = null; 
            } else { errorMsg(result.message || "File upload failed on server."); }
        } catch (error) { errorMsg("File upload error. Check the UPLOAD_URL and API Key.", error?.response?.data || error); }
    };
    ...
    const handleSubmit = async (e, dataWithTenant) => {
        ...
        try {
            ...
            if (response.data.success) { successMsg("Category created successfully!"); getAllTourCategory(); } else { errorMsg(response.data.message || "Failed to create category."); }
        } catch (error) { errorMsg("Submission error.", error?.response?.data || error); }
    };
    ...
    const handleUpdate = async (e, dataWithTenant) => {
        ...
        try {
            ...
            if (response.data.success) { successMsg("Category updated successfully!"); getAllTourCategory(); } else { errorMsg(response.data.message || "Failed to update category."); }
        } catch (error) { errorMsg("Update error.", error?.response?.data || error); }
    };
    
    // --- Bulk Delete Implementation (Unchanged logic) ---
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            const deletePromises = selectedIds.map(id => 
                axios.delete(`${CATEGORY_API_URL}${id}?tenant_id=2`, {
            ...
            successMsg("Bulk deletion process completed.");
            setSelectedIds([]);
            getAllTourCategory(); 
        } catch (error) {
    ...
    // --- Action Button Handlers (Unchanged logic) ---
    ...
    const handleDelete = async (categoryId) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                getAllTourCategory();
            } else { errorMsg(response.data.message || "Failed to delete category."); }
        } catch (error) { errorMsg("Delete error.", error?.response?.data || error); }
    };

    const handleDuplicate = (category) => {
        const { id, _id, ...rest } = category; 
        const imagesForDuplicate = rest.image.map(getFullImageUrl); 

        setCategoryData({ 
            ...rest, name: `${category.name} (Copy)`, slug: `${category.slug}-copy`,   
            tenant_id: 2, image: imagesForDuplicate 
        });
    ...
    const handleView = (category) => {
        setCategoryData({ ...category });
        setIsViewOnly(true); setIsUpdate(false); setValidation({}); setIsModalOpen(true);
    };
    ...
    const handleOpenCreateModal = () => {
        setCategoryData({ name: '', slug: '', description: '', image: [], tenant_id: 2 });
        setIsUpdate(false); setIsViewOnly(false); setValidation({}); setIsModalOpen(true);
    };
                                    ...
                                    <tr key={id}> 
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div className="flex items-center space-x-2 action-icons">
                                                ...
                                                <button onClick={() => handleView(category)} title="View" className="text-green-600 hover:text-green-800"><Eye size={18} /></button>
                                            </div>
                                        </td>
                                    </tr>
                                );

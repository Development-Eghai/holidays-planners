...
const TripCard = ({ trip, onSendQuery }) => {
    ...
    // ✅ FIX 1: Use the standardized price field from parent component (DestinationCards)
    const finalPriceDisplay = trip.final_price_display || "N/A"; 
    
    // ✅ FIX 2: Use the standardized discount field
    const discountAmount = trip?.pricing?.discount || 0; 
    
    // Fallback if standardized discount isn't present (e.g., legacy data)
    const displayDiscount = discountAmount > 0 ? discountAmount : (trip.pricing?.customized?.discount || 0);
